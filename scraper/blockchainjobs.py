...
class BlockchainjobsScraper(BaseScraper):
    """Scrape blockchain jobs from BlockchainJobs.io."""

    def __init__(self):
        super().__init__("blockchainjobs.io")

    async def fetch(self) -> List[Dict]:
        """Fetch BlockchainJobs listings."""
        jobs = []

        try:
            url = "https://blockchainjobs.io/jobs"
            ...
            job_elements = tree.xpath("//div[contains(@class, 'job-listing')]")

            for element in job_elements:
                try:
                    title_elem = element.xpath(".//h3[@class='job-title'] | .//a[contains(@class, 'job-link')]")
                    title = title_elem[0].text_content().strip() if title_elem else "Untitled"

                    company_elem = element.xpath(".//span[@class='company-name'] | .//a[contains(@class, 'company')]")
                    company = company_elem[0].text_content().strip() if company_elem else "Unknown"

                    location_elem = element.xpath(".//span[@class='location'] | .//span[contains(text(), 'Remote')]")
                    ...
                    if not job_url.startswith("http"):
                        job_url = f"https://blockchainjobs.io{job_url}"
